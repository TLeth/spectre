...
class Skeleton {
  ...
  Skeleton(this.name, int length) : boneList = new List<Bone>(length) {
...
abstract class SkeletonPoser {
  ...
  pose(Skeleton skeleton, SkeletonAnimation animation, PosedSkeleton posedSkeleton, double t);
}
...
class SimpleSkeletonPoser implements SkeletonPoser {
  ...
  void updateGlobalTransform(Bone bone, Float32List parentTransform, SkeletonAnimation animation, PosedSkeleton posedSkeleton, double t) {
    ...
    for (int i = 0; i < bone.children.length; i++) {
      ...
      updateGlobalTransform(childBone, globalTransform, animation, posedSkeleton, t);
    }
  }
...
class SIMDSkeletonPoser implements SkeletonPoser {
  ...
  void mul44SIMD(Float32x4List out, Float32x4List a, Float32x4List b) {
    ...
    out[0] = b0.shuffle(Float32x4.XXXX) * a0 + b0.shuffle(Float32x4.YYYY) * a1 + b0.shuffle(Float32x4.ZZZZ) * a2 + b0.shuffle(Float32x4.WWWW) * a3;
    out[1] = b1.shuffle(Float32x4.XXXX) * a0 + b1.shuffle(Float32x4.YYYY) * a1 + b1.shuffle(Float32x4.ZZZZ) * a2 + b1.shuffle(Float32x4.WWWW) * a3;
    out[2] = b2.shuffle(Float32x4.XXXX) * a0 + b2.shuffle(Float32x4.YYYY) * a1 + b2.shuffle(Float32x4.ZZZZ) * a2 + b2.shuffle(Float32x4.WWWW) * a3;
    out[3] = b3.shuffle(Float32x4.XXXX) * a0 + b3.shuffle(Float32x4.YYYY) * a1 + b3.shuffle(Float32x4.ZZZZ) * a2 + b3.shuffle(Float32x4.WWWW) * a3;
  }

  void updateGlobalTransform(Bone bone, Float32x4List parentTransform, SkeletonAnimation animation, PosedSkeleton posedSkeleton, double t) {
    int boneIndex = bone._boneIndex;
    final Float32x4List nodeTransform = _scratchMatrix;
    final Float32x4List globalTransform = posedSkeleton.globalTransforms4[boneIndex];
    BoneAnimation boneData = animation.boneList[boneIndex];
    if (boneData != null) {
  ...
  void updateSkinningTransform(PosedSkeleton posedSkeleton, Skeleton skeleton) {
    for (int i = 0; i < skeleton.boneList.length; i++) {
      ...
      mul44SIMD(skinningTransform, skeleton.globalOffsetTransform4, skinningTransform);
    }
  }
